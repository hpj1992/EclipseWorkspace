...
public class Operations {
	...
	public void createVMForHadoop(ESXI esxi,int count,String datasourceName,String targetTemplate,String newVMName) throws RemoteException, MalformedURLException, InterruptedException
	{
		...
		HostSystem targetHost = getTargetHost(hostIp,rootFolder);
		ResourcePool targetPool = getTargetResourcePool(resourcePool,rootFolder,targetHost);
		...
		boolean isTemplatedCreated = createNewVMFromTemplate(vmTemplate, targetHost, targetDatastore, targetPool, si, boot,newVMName);
	}
	...
	public ResourcePool getTargetResourcePool(String resourcePool,Folder rootFolder, HostSystem targetHost) throws InvalidProperty, RuntimeFault, RemoteException
	{
		...
		if (resourcePool == null || resourcePool.length() == 0) {
			...
			System.out.println("host of slave"+hostResource.getName());
			System.out.println(hostResource.getName());
	...
	public boolean createNewVMFromTemplate(VirtualMachine vmTemplate,HostSystem targetHost, Datastore targetDatastore, ResourcePool targetPool,ServiceInstance si,VirtualMachineBootOptions boot,String newVMName) throws VmConfigFault, TaskInProgress, CustomizationFault, FileFault, InvalidState, InsufficientResourcesFault, MigrationFault, InvalidDatastore, RuntimeFault, MethodFault, RemoteException, InterruptedException
	{
		...
		if (vmTemplate != null) {
			...
			runTask(vmTemplate.cloneVM_Task((Folder) vmTemplate.getParent(), newVMName, cloneSpec), wait);
...
/*	
	...
	public VM getHostDetails(Folder rootFolder, VM vm) throws InvalidProperty, RuntimeFault, RemoteException
	{
			...
			for(ManagedEntity temp: mes)
			{
				...
				{
					if(vms[i].getName().equalsIgnoreCase(vm.getName()))
					{
						vm.setHost(ds.getName());
						break;
					}
				}
