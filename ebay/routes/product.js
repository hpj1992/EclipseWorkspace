...
exports.postBid = function(req,res)
{
	...
	var userId = 1 ;//req.session.loggedInUser;
			...
			{	/*console.log("***************final Results************");
				...
				*/
				...
				res.render("bidHistory", { totalBids : totalBids , totalUsers : totalUsers , bids : results.bids,
					...
					productId : results.auction[0].productId , auctionId : results.auction[0].auctionId
				});
...
function getBidInfo (callback,auctionId)
{
	...
	var getBidInfo = "SELECT b.bidId,b.auctionId,b.bidPrice ,b.date,b.bidBy,p.firstName" +
			...
			" ebay.customer c   ON b.bidBy = c.customerId INNER JOIN " +
			" ebay.person p ON c.personId = p.userId where b.auctionId = '"+auctionId+"' ORDER BY Date(date) desc,Time(date) desc";
