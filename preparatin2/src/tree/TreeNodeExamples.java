...
public class TreeNodeExamples {
	...
	public static void main(String[] args) {
		...
		TreeNode newT = createMinimalBST(data, 0, 6);
		TreeNode ans = createBST(data);
		preOrderRecursive(ans);
	}
	...
	static boolean isBalanced(TreeNode root) {
		...
		int diff = Math.abs((getHeight(root.getLeft()) - getHeight(root
				.getRight())));
		if (diff > 1)
			return false;
		else
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Alternative to check whether the tree is balanced or not, O(N) time and
	// O(H) space
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Create BST, O(N) time [Alternative: insert node recursive method but that
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Check whether two trees are identical or not, O(N) time and O(logN) space
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Check whether the tree is BST min is -2^31 & max is 2^31 -1, O(N) time
	// and O(logN) space because of recursive calls
	public static boolean checkBSTAlt(TreeNode root, Integer min, Integer max) {
		...
		return checkBSTAlt(root.getLeft(), min, root.getData())
				&& checkBSTAlt(root.getRight(), root.getData(), max);
	}
	...
	static ArrayList<TreeNode> postOrderTraversal(TreeNode root) {
		...
		while (!stack.isEmpty()) {
			...
			// it and pop stack, otherwise, keep going down
			if (prev == null || prev.getLeft() == curr
					|| prev.getRight() == curr) {
				...
				// right child, if yes, push it to stack, otherwise, process
				...
				// go up the tree from right node, after coming back from right
				// node, process parent node and pop stack.
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Build binary tree using in order and post order traversal, O(N) time and
	// O(logN) space
	static TreeNode buildTree(int[] inOrder, int inStart, int inEnd,
		...
		root.setLeft(buildTree(inOrder, inStart, k - 1, postOrder, postStart,
				postStart + k - (inStart + 1)));
		root.setRight(buildTree(inOrder, k + 1, inEnd, postOrder, postStart + k
				- inStart, postEnd - 1));
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Flatten the binary tree in linked list. i.e. all left or right are nulls,
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Create N link list for N levels, O(N) time and O(logN) space. DFS Approach

	static void createLinks(ArrayList<LinkedList<TreeNode>> levelList,
	...
	// Alternative approach, O(N) time and O(1) space. BFS Approach
	...
	static boolean removeNode(TreeNode parent, TreeNode current, int data) {
		...
		if (data < curr) {
			...
			if (current.getLeft() != null && current.getRight() != null) {
				...
				parent.setLeft((current.getLeft() != null) ? current.getLeft()
						: current.getRight());
			} else if (parent.getRight() == current) {
				parent.setRight((current.getLeft() != null) ? current.getLeft()
						: current.getRight());
			}
		}
		return true;
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Find successor of the given node inorder traversal Assume you can go to
	// parent
	...
	// Most optimized LCA for BST: O(h) , h is height
	/*
	 * public static Node lowestCommonAncestor(Node root, Node a, Node b) { if
	 * (root == null || a == null || b == null) { return null; }
	 * 
	 * if (Math.max(a.data, b.data) < root.data) { // both nodes are on the left
	 * return lowestCommonAncestor(root.left, a, b); } else if (Math.min(a.data,
	 * b.data) > root.data) { // both nodes are on the right return
	 * lowestCommonAncestor(root.right, a, b); } else { // the nodes are on
	 * separate branches return root; } }
	 */

	// -----------------------------------------------------------------------------------------------------------------
	// Check whether the given tree is sub tree of other(identical in shape as
	// well as values), O(N + KM), K is the no of occurrences of r2's root in r1
	...
	// -----------------------------------------------------------------------------------------------------------------
	// Given a node and value find out the paths which will sum to the given
	// value.
	...
	// -----------------------------------------------------------------------------------------------------------------
	/*
	 * Very simple code..also prints the path-
	 * 
	 * bool printpath(struct node*root, int sum){ if(!root)return (sum==0);
	 * if(printpath
	 * (root->left,sum-root->data)||printpath(root->right,sum-root->data)) {
	 * cout<<root->data<<" -> "; return true; } return false; }
	 */
}
